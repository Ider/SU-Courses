...
std::string MetaGenerator::GeneratePackInfo(std::string& filePath)
{
	...
	if(filePath[filePath.size()-1] == 'h')
		...
		tag="implement";
	std::string fileName = filePath.erase(0,filePath.find_last_of('\\')+1);
	packInfo.addSibling(xmlElem(tag,fileName));
...
{
	EmbraceReferences();
	xmlElem owner("owner",owner);

	std::string attribe = "name=\""+packName+"\"";
	std::string tag = xmlElem::makeTag("package",attribe);
	xmlElem	combine(tag,packInfo.xmlStr()+references.xmlStr()+owner.elemStr());
